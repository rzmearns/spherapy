import unittest
import numpy as np
import thermpy.solver.mission.timespan as TS
...
class TestTimeSpan(unittest.TestCase):
	...
	def test_init(self):
		...
		dt_week = firstweek.as_datetime()
		astropy_week = firstweek.as_astropy()
		
		self.assertEqual(len(dt_week), 7)
		...
		# Assert the last day for each is Sept 16, 1996
		self.assertEqual(dt_week[-1], dt.datetime(1996, 9, 16))
		self.assertTrue(astrotime.Time('1996-09-16').isclose(astropy_week[-1]))
	...
	def test_parsing(self):
		...
		a variety of units
		'''
		t0 = dt.datetime(2016, 12, 30, 0, 0, 1)
		...
		self.assertEqual(40, len(TS.TimeSpan(t0, '1.5M', '1H')))
		
		# Test non integer value of period
		self.assertEqual(90, len(TS.TimeSpan(t0, '1M', '1.5H')))

		# Test integer number of steps
		self.assertEqual(34, len(TS.TimeSpan(t0, '1.75M', '1H')))
		...
		self.assertNotEqual(len(TS.TimeSpan(t0, '1S', '2d')), astro_delta.value)

		# Check leap year handling
		self.assertEqual(366, len(TS.TimeSpan(dt.datetime(2016, 1, 1), '1d', '1y')))
	...
	def test_time_zones(self):
		...
		self.assertIsNone(np.testing.assert_allclose(np.vectorize(lambda x: x.total_seconds())(timespan.as_datetime() - timespan_tz.as_datetime()),
													np.vectorize(lambda x: x.total_seconds())(dt.timedelta(hours=6, minutes=30) * np.ones(len(timespan)))))
		...
		self.assertIsNone(np.testing.assert_allclose(np.vectorize(lambda x: x.total_seconds())(timespan.as_datetime() - timespan_tz.as_datetime()),
													np.vectorize(lambda x: x.total_seconds())(dt.timedelta(hours=-4, minutes=-45) * np.ones(len(timespan)))))
	...
	def test_seconds_since_start(self):
		...
		seconds_elapsed = timespan.seconds_since_start()
		self.assertEqual(len(seconds_elapsed), 24)
		self.assertEqual(seconds_elapsed[0], 0)
		np.testing.assert_almost_equal(seconds_elapsed[1], 3600)
		np.testing.assert_almost_equal(seconds_elapsed[-1], 82800)
